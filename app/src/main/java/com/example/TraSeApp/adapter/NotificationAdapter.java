...
import com.example.TraSeApp.model.Noti;
...
public class NotificationAdapter extends RecyclerView.Adapter<NotificationAdapter.ImageViewHolder> {
    ...
    private List<Noti> mNoti;

    public NotificationAdapter(Context context, List<Noti> noti){
        mContext = context;
        mNoti = noti;
    }
    ...
    public void onBindViewHolder(@NonNull final NotificationAdapter.ImageViewHolder holder, final int position) {
        ...
        final Noti noti = mNoti.get(position);

        holder.text.setText(noti.getText());

        getUserInfo(holder.image_profile, holder.username, noti.getUserid());

        if (noti.isIspost()) {
            holder.post_image.setVisibility(View.VISIBLE);
            getPostImage(holder.post_image, noti.getPostid());
        } else {
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (noti.isIspost()) {
                    ...
                    editor.putString("postid", noti.getPostid());
                    editor.apply();

                } else {
                    SharedPreferences.Editor editor = mContext.getSharedPreferences("PREFS", MODE_PRIVATE).edit();
                    editor.putString("profileid", noti.getUserid());
                    editor.apply();
    ...
    private void getUserInfo(final ImageView imageView, final TextView username, String publisherid){
        ...
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                Glide.with(mContext).load(user.getImgUrl()).into(imageView);
